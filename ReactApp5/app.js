...
var NotePad = React.createClass({
	...
	onDelete: function() {
		if (!this.state.isConfirming) 
			...
			this.props.onDeletedNote(this.props.notepad.selectedId);
				...
				<div>
					{
						!this.state.isConfirming ? 
							...
							<div>
								<button onClick={this.onDelete} className="myButton">Confirm</button>
								<button onClick={this.onCancelDelete} className="myButton">Cancel</button>
							</div>
					}

					{
						!this.state.isConfirming && this.props.notepad.notes.length != 0 && this.props.notepad.selectedId != null ? 
							<button onClick={this.onDelete} className="myButton">Delete Note</button> : 
							null
					}	
				</div>
				{editor}
